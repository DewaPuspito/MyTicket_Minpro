...
import { CalendarDays, MapPin, Ticket, User, CreditCard, ArrowLeft, Banknote, Smartphone } from "lucide-react";
...
export default function TicketPage() {
    ...
    if (!event) {
        return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-purple-50 to-blue-50 text-center p-6">
                <h1 className="text-3xl font-bold">Event Not Found</h1>
            </motion.div>
        );
    }

    const formatPrice = (price: string) => `Rp. ${price.replace(/\B(?=(\d{3})+(?!\d))/g, ".")}`;
    const basePrice = Number(event.price.toString().replace(/\./g, "")) * ticketCount;
    const discountedPrice = basePrice - (basePrice * (discount / 100));

    return (
        <motion.div variants={staggerContainer()} initial="hidden" animate="show" className="min-h-screen bg-gradient-to-br from-[#f8fafc] to-[#e2e8f0] p-6">
            <div className="max-w-6xl mx-auto">
                <motion.div variants={fadeIn("up", "tween", 0.4, 1)} className="bg-white shadow-2xl rounded-2xl overflow-hidden transition-shadow hover:shadow-3xl">
                    <div className="bg-[#00214D] p-8 text-white">
                        <div className="flex items-center justify-between">
                            <Button className="flex items-center gap-2 bg-white/10 backdrop-blur-sm hover:bg-white/20 rounded-xl px-6 py-3" onClick={() => router.back()}>
                            ...
                            <div className="border rounded-xl p-6 shadow-sm bg-indigo-50">
                                ...
                                <div className="space-y-4 mb-6">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-medium">{formatPrice(event.price.toString())}</span>
                                    </div>
                                    {/* <div className="flex justify-between">
                                    ...
                                    {discount > 0 && (
                                        <div className="flex justify-between text-green-600">
                                            ...
                                            <span>-{formatPrice((basePrice * (discount / 100)).toString())}</span>
                                        </div>
                                    )}
                            ...
                            <form className="space-y-8">
                                ...
                                <div>
                                    ...
                                    {discount > 0 && !referralError && (
                                        <p className="mt-2 text-green-600 font-medium">Voucher applied! {discount}% discount.</p>
                                    )}

                                    {referralError && (
                                        <p className="mt-2 text-red-600 font-medium">{referralError}</p>
                                    )}
                                    ...
                                    <div className="space-y-6">
                                        {attendees.map((_, index) => (
                                            <div key={index} className="bg-white p-4 rounded-lg border shadow-sm">
                                                <h4 className="font-medium mb-4 text-gray-700">Attendee #{index + 1}</h4>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        {['BCA', 'Mandiri', 'Gopay', 'OVO'].map((method) => (
                                            <div
                                                key={method}
                                                className={`p-4 border rounded-xl cursor-pointer transition-all ${selectedPayment === method ? 'border-indigo-500 bg-indigo-50' : 'hover:border-gray-300'}`}
                                                onClick={() => setSelectedPayment(method)}
                                            >
                                                <div className="flex items-center">
                                                    <div className={`w-6 h-6 mr-3 flex items-center justify-center rounded-full ${selectedPayment === method ? 'bg-indigo-500 text-white' : 'bg-gray-200'}`}>
                                    ...
                                    {selectedPayment && (
                                        <div className="mt-6 space-y-4">
                                            <div className="flex items-center border rounded-lg p-3 bg-gray-50">
                                                <input
                                                    type="text"
                                                    placeholder={selectedPayment === 'Gopay' || selectedPayment === 'OVO' ? 'Phone Number' : 'Virtual Account Number'}
                                                    className="w-full outline-none bg-transparent"
                                                    required
                                                />
                                            </div>
