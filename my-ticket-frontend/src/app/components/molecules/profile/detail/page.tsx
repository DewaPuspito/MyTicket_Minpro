            ...
            <div className="w-full max-w-md bg-white/10 backdrop-blur-lg rounded-2xl p-8 shadow-2xl border border-white/20">
                ...
                <div className="space-y-5">
                    ...
                    {(profile.role?.toUpperCase() === 'CUSTOMER' || profile.role === 'Customer') && (
                        <div className="space-y-1">
                            <label className="text-sm text-white/90 font-medium">Your Coupon Codes</label>
                            <div className="w-full bg-white/5 text-white border border-white/20 rounded-lg px-4 py-3 focus:outline-none cursor-default">
                                {Array.isArray(profile.codes) ? (
                                    profile.codes.map((code, index) => (
                                        <div key={index}>{code}</div>
                                    ))
                                ) : typeof profile.codes === 'string' && profile.codes.length > 0 ? (
                                    profile.codes.split(',').map((code, index) => (
