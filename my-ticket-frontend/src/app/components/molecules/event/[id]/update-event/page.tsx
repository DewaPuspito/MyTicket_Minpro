    ...
    const fetchEventData = async () => {
      try {
        ...
        setFormData({
          ...
          remainingTickets: eventData.available_seats,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        if (error.response) {
          alert(`Error: ${error.response.data.message || 'Terjadi kesalahan saat memperbarui event'} ${error.response.data.error? error.response.data.error : ''}`);
    ...
    <div className="min-h-screen bg-gradient-to-r from-[#002459] to-[#0d1e4a] flex items-center justify-center p-6">
      <div className="w-full max-w-3xl bg-white/10 backdrop-blur-lg rounded-2xl p-8 shadow-2xl border border-white/20">
        ...
        <div className="flex flex-col items-center text-center space-y-3 mb-8">
          ...
          />
          <h2 className="text-3xl font-bold text-white/90">Update Event</h2>
        </div>
