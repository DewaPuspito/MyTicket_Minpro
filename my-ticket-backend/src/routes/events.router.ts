...
export class EventRouter {
  ...
  private routes(): void {
    ...
    this.router.put('/event/:id', AuthenticationMiddleware.verifyToken, EventMiddleware.findEvent, AuthenticationMiddleware.checkOwnership, AuthorizationMiddleware.allowRoles('EVENT_ORGANIZER'), ValidationMiddleware.validate({body: eventSchema.body, params: eventSchema.params, partial: true}), this.eventController.update.bind(this.eventController))
    this.router.delete('/event/:id', AuthenticationMiddleware.verifyToken, EventMiddleware.findEvent, AuthenticationMiddleware.checkOwnership, AuthorizationMiddleware.allowRoles('EVENT_ORGANIZER'), this.eventController.delete.bind(this.eventController))
  }
}